...
The `Auto-correct` feature enables misspelt commands entered by users to be auto-corrected by the application,
...
of alphabets, transposing of alphabets and Substitution of alphabets. For the sake of efficiency, the edit distance is limited to 2 (refer to `editDistance1`, and `checkMisspeltWords`).

This is an example of how the misspelt words are prediction by adding one more alphabet:
[source,java]
----
...
Auto-Correct is only utilized in `AddressBoonParser` class to check each input command, and correct the misspelt command which has an edit distance smaller than or equal to 2.
...
The query of weather information is closely related to the location. The location is specified by WOEID, so in this application we take the WOEID of Singapore by default. +
...
The overview of the weather displyed on the application is as follows:

image::WeatherDisplay.png[width="200" height="30"]
...
=== Autocomplete [[autocomplete-feature]] mechanism
...
The interaction between `AutoCompleteUtil` and `LogicManager` classes can be seen from the following sequence diagram:
...
=== Extended Autocomplete [[extended-autocomplete-feature]] mechanism
...
This command can be illustrated by the following diagram, the prefix is prompt automatically when the key `Control` is pressed on teh keyboard.

image::AutoCompleteExtended.png[width="800"]

==== Prerequisites/Dependencies

* The hotkey for launching `extended autocomplete` is indicated in the `CommandBox` class.
Currently, the hotkey is `Ctrl`.
