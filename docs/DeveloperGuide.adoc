...
The `Autocomplete` feature eables commands to be automatically suggested and
completed for users. It is invoked in the `LogicManager` class.

*Prerequisites/Dependencies*

* The hotkey for lauching Autocomplete is indicated in the `CommandBox` class.
...
* Although the autocomplete feature currently does not support autocompletion
...
*Citeria for autocomplete*

* `COMMAND_WORD` starts with user-entered prefix (e.g. del matchs `Delete` commmand).
*  `If no prefix is entered`, all commands are displayed to the user.
...
** 3b1. AddressBook will not respond.
