...
the `mapOfCommandFormats` map in the  `Command` class.
...
* The `COMMAND_WORD`, `MESSAGE_USAGE`  and `AUTOCOMPLETE_FORMAT` fields must exist in all class files that represent commands.
...
* The `mapOfCommandFormats` map in the `Command` class must contain `<command_class.COMMAND_WORD, command_class.AUTCOMPLETE_FORMAT>`
as `<key, value>` pairs. An example of such a pair is `<AddCommand.COMMAND_WORD, AddCommand.AUTOCOMPLETE_FORMAT>`.
* The `mapOfCommandHelp` map in the `Command` class must contain `<command_class.COMMAND_WORD, command_class.MESSAGE_USAGE>`
as `<key, value>` pairs. An example of such a pair is `<AddCommand.COMMAND_WORD, AddCommand.COMMAND_USAGE>`.
* If the developer forgets to perform the above steps, `AddressBook` will work
