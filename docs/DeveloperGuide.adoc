...
The `Auto-correct` feature enables mis-spelt commands entered by users to be auto-corrected by the application,
and perform the correct command.
...
* Auto-correct will only work for mis-spelt words within edit distance of 2.
* Auto-correct is based on four types of prediction of mis-spelt words, namely, addition, deletion, switch, and substitution.
* Alias will not be auto-corrected as it consists only one or two alphabets.
* The logic for auto-correcting commands is implemented in AutoCorrectCommand class.

==== Design Consideration

**Aspect:** Implementation of `Auto-correct` +
...
**Cons:** The prediction by first alphabet limits the auto-correction on those commands that have first alphabet wrong.
Edit distance 2 does not cover many other mis-spelt commands. +

**Alternative 2:** Read all the available file from an external file when the application begins. +
**Pros:** It would be easier for developers to maintain the auto-correct feature as it is easier to update new commands into a file rather than in a method. +
...
The `Auto-complete` feature enables commands to be automatically suggested and
completed for users. It is invoked in the `LogicManager` class.
...
the `listOfAvaliableCommands` list in the  `Command` class.
...
The `Delete` multiple contact feature enables user to delete more than one contact in a single command. It is
...
|`* * *` |university student |backup and restore my contacts |recover my contacts in case of unexpected incidents.

|`* * *` |person with a large social circle |filter my contacts by multiple criteria |search for specific contacts more easily

|`* * *` |careless user |have autocorrection for my misspelt words |my contact details will be properly spelt

|`* * *` |user |add multiple phone numbers to a person |know different ways of contacting one person

|`* * *` |busy user |sort the address book alphabetically |locate my contacts easily

|`* * *` |user |load more than 100,000 persons at one time |load all my schoolmates in NUS

|`* * *` |user |search friends by tags |search friends by group

|`* *` |busy businessma |delete multiple users simultaneously|perform bookeeping more easily

|`* *` |user |delete a person |remove entries that I no longer need

|`* *` |businessman |share my contact |easier for my client to contact me

|`* *` |user with many contacts |listed contacts to be arranged into several pages |easier for me to read

|`* *` |user with bad memory of commands |autocomplete commands|type lengthy commands in full

|`* *` |user |check my most frequently contacted list of people |add them to my favourite list, or located them more easily

|`*` |user |merge my address book with another address book|add contacts in bulk

|`*` |user that loves social media |search for my contact's facebook profile via phone number |connect with them on a deeper level

|`*` |user with many persons in the address book |sort persons by name |locate a person easily

|`*` |user |access address boook through slack |access address book quickly and conveniently

|`*` |user loves emoji |add emoji as special tags to the contact cards |have an interesting address book contacts

|`*` |user with non-english as their native speaking language |add non-english names of friends |have localization for the language

|`*` |user likes customization |change the colour theme |make my address book more personalised

|`*` |forgetful person |display avatars for each contact|recall who a particular person is.

|`*` |lazy user |voice out the commands rather than typing |be more efficient at adding contacts

|`*` |user with many friends |record the appointments with friends and be notified y address book |be reminded to attend the appointments

|`*` |artistic person |change the font size and font family of my contacts' records|have a more aesthetically pleasing addressbook interface.

|`*` |music lovers |add different sound for different events|have a better feedback system to the user.

|`*` |traveller |add weather forecast|plan my trips efficiently.

|`*` |forgetful person |push notifications for upcoming tasks| i can be reminded of my priorities and tasks.

|`*` |impatient person |date picker| so that i can input the format of the date correctly.

|`*` |lazy person |have keyboard shortcuts| so i can perform the functions without typing the whole command.
...
[discrete]
=== Use case: Search contact on Facebook

*MSS*

1.  User requests to list persons
2.  AddressBook shows a list of persons
3.  User requests to search a person's contact on facebook
4.  AddressBook shows the link to search result
+
Use case ends

*Extensions*

* 3a. The person is not on Facebook
+
Use case ends.

[discrete]
=== Use case: Show contacts by pages

*MSS*

1.  User requests to list persons
2.  AddressBook shows a list of persons
3.  User requests to list by pages
4.  AddressBook shows the list in pages
+
Use case ends.
...
1.  User types a command halfway and presses the autocomplete hotkey.
