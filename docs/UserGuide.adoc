...
. Enter `multifilter` into the command bar followed by one or more format fields.
...
Enter some text into the command bar (as shown below) and press kbd:[TAB].
...
. Press the kbd:[Ctrl] key. The command's format is automatically filled in the command bar for you.
