...
=== Locating persons by tag: `search` + TAG_NAME [Since v1.1]
...
Format: kbd:[CONTROL] + kbd:[COMMAND_WORD_ALIAS]

****
* The keyborad shortcuts are created only for commands with heavy text input.
* The commands with keyboard shortcuts are: `add`, `multifilter`, `edit`, `search`, `delete`, and `clear`.
****
